...
// Single interleaved buffer: vec2 center + float size + vec3 color = 6 floats
const STRIDE = 6 * 4; // 6 floats, 4 bytes each
const instanceBuffer = gl.createBuffer();
gl.bindBuffer(gl.ARRAY_BUFFER, instanceBuffer);

// Center (vec2) at offset 0
gl.enableVertexAttribArray(aCenter);
gl.vertexAttribPointer(aCenter, 2, gl.FLOAT, false, STRIDE, 0);
gl.vertexAttribDivisor(aCenter, 1); // Advance once per instance

// Size (float) at offset 8
...
function spawnAtomsFromCargo(cargo, x, y, spread = 30) {
    for (const type of cargo.types()) {
        ...
        const atom = { x: 0, y: 0, type: '', amount: 0, color: 0, phase: 0 };
...
function destroyDrone(drone) {
    if (!drone) return;
    if (drone.cargo && !drone.cargo.isEmpty()) {
...
function fireSingularity(block) {
    ...
    const sw = { x: 0, y: 0, radius: 0, maxRadius: 0, life: 0, alpha: 1, type: 'normal' };
    sw.x = block.x; sw.y = block.y; sw.radius = 0; sw.maxRadius = range; sw.life = 1500; sw.alpha = 1; sw.type = 'singularity';
    shockwaves.push(sw);

    const rift = { x: 0, y: 0, radius: 0, maxRadius: 0, life: 0, maxLife: 0, phase: 0 };
    ...
    for (const a of aliens) {
        if (Math.hypot(a.x - block.x, a.y - block.y) < range) {
            for (let j = 0; j < Math.floor(a.size * 12); j++) {
                const p = { x: 0, y: 0, angle: 0, radius: 0, speed: 0, brightness: 0, size: 0, life: 0, maxLife: 0, ownerX: 0, ownerY: 0 };
                p.x = a.x; p.y = a.y;
...
function moveToward(obj, targetX, targetY) {
    const angle = Math.atan2(targetY - obj.y, targetX - obj.x) + (Math.random() - 0.5);
...
function orbitAround(obj, targetX, targetY) {
    const angle = Math.atan2(obj.y - targetY, obj.x - targetX) + Math.PI/2 + (Math.random() - 0.5);
    const moveSpeed = obj.speed * (elapsedTime / 16);
    obj.x += Math.cos(angle) * moveSpeed;
    obj.y += Math.sin(angle) * moveSpeed;
    obj.x += (targetX - obj.x) * 0.008 * (elapsedTime / 16);
    obj.y += (targetY - obj.y) * 0.008 * (elapsedTime / 16);
}
...
function updateDrone(drone) {
    ...
    if (!drone.cargo.isEmpty()) {
        for (const { cargo, block } of storages) {
            ...
            const dist = Math.hypot(block.x - drone.x, block.y - drone.y);
            if (dist < bestDist) {
                bestDist = dist;
                bestTarget = block;
                isStorage = true;
...
function update() {
    ...
    for (const b of blocks) {
        ...
        // Core defense
        if (b.type === 'core') {
            b.cooldown -= elapsedTime;
            if (b.cooldown <= 0) {
                let closest = null, minD = def.range;
                for (const a of aliens) {
                ...
                if (closest) {
                    ...
                    const p = { x: 0, y: 0, vx: 0, vy: 0, damage: 0, color: 0, life: 0 };
                    p.x = b.x;
                    p.y = b.y;
            ...
            if (b.cooldown <= 0) {
                ...
                if (ownedCount < def.droneMax) {
                    if (unowned) {
                        ...
                        const d = { owner: null, x: 0, y: 0, cargo: null, speed: DRONE.speed };
                        d.cargo = Cargo(DRONE.capacity._capacity);
                        d.owner = b;
                        d.x = b.x;
                        d.y = b.y;
                        drones.add(d);
...
function render() {
    ...
    for (const a of atoms) {
        ...
        addQuad(a.x, a.y, B / 2, c[0], c[1], c[2]);
    }
    ...
    for (const p of singularityParticles) {
        ...
        addQuad(p.x, p.y, B * p.size, c[0], c[1], c[2]);
    }
    ...
    for (const b of blocks) {
        if (b.type === 'laser' && b.laserTarget && aliens.has(b.laserTarget)) {
            ...
            for (let i = 0; i < steps; i++) {
                ...
                addQuad(b.x + dx * t, b.y + dy * t, B, c[0], c[1], c[2]);
            }
        }
    }
    ...
    for (const d of drones) {
        const c = C(35, 1.4);
        addQuad(d.x, d.y, B / 2, c[0], c[1], c[2]);
    }
...
const coreBlock = {
    ...
    animPulse: 0,
    cooldown: 0,
    cargo: CORE_BLOCK.capacity.clone(true),
    buildCost: Cargo(),
};
